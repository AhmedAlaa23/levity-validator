...
const checkStringLengthRange = (str, min=null, max=null)=>{
...
const isAlphaNum = (str="", options={min: 0, max: 10000, locale: 'en-US'})=>{
	...
	let regExpPattern = alphaLocaleRegExpUni[options.locale] || options.locale;
...
const isAlphaNumUnderscore = (str="", options={min: 0, max: 10000, locale: 'en-US'})=>{
	if(!isString(str)){return false;}
	if(!checkStringLengthRange(str, options.min, options.max)){return false;}
...
export 
{isDefined,
isString, isAlpha, isAlphaNum, isAlphaNumSpace, isAlphaNumUnderscore, isEmptyString,
isNum, isInt};